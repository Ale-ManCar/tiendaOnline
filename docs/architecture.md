...
Nova Store uses a modular-monolith strategy. The React/Vite storefront remains independently deployable, while `server/` provides a NestJS REST API backed by PostgreSQL and Prisma. API routes are versioned under `/api/v1`. Database access is isolated in a global database module; authentication, catalog, and orders are now backend-owned modules. HashRouter keeps the storefront compatible with GitHub Pages during migration.
...
Checkout order creation is authenticated and server-calculated. The client submits variant IDs, quantities, shipping data, and payment method; the API calculates prices, tax, totals, validates available stock, decrements stock, and persists the order in a transaction.
