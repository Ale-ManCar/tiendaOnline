...
export async function registerAccount(name:string,email:string,password:string){try{return mapUser((await request('/auth/register',{name,email,password})).user)}catch(error){if(isApiUnavailable(error))return registerDemoUser(name,email,password);throw error}}
export async function loginAccount(email:string,password:string){try{return mapUser((await request('/auth/login',{email,password})).user)}catch(error){if(isApiUnavailable(error))return loginDemoUser(email,password);throw error}}
export async function restoreSession(){
  try{return mapUser((await apiRequest<AuthResponse>('/auth/me')).user)}catch(error){
    if(isApiUnavailable(error))return restoreDemoSession();
    if (!(error instanceof ApiError) || error.status !== 401) throw error;
    try{await request('/auth/refresh');return mapUser((await apiRequest<AuthResponse>('/auth/me')).user)}catch{return null}
  }
}
export async function logoutAccount(){try{await apiRequest<void>('/auth/logout',{method:'POST'})}catch(error){if(!isApiUnavailable(error))throw error}finally{logoutDemoUser()}}
