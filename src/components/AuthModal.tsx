...
export function AuthModal({ open, onClose, notify }: { open:boolean; onClose:()=>void; notify:(toast:ToastState)=>void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="auth-modal" onMouseDown={e=>e.stopPropagation()} aria-modal="true" role="dialog" aria-labelledby="auth-title"><button className="icon-button modal-close" onClick={onClose} aria-label="Close"><X/></button><div className="auth-head"><span className="eyebrow">{storeConfig.name.toUpperCase()}</span><h2 id="auth-title">{mode==='login'?'Welcome back':'Create your account'}</h2><p>{mode==='login'?'Sign in to continue shopping.':'Register to save your cart and orders.'}</p></div><form onSubmit={submit} className="form-stack">
